...
// `pageType` swaps WebPage for a subtype such as CollectionPage.
export function structuredData({ site, url, title, description, breadcrumbs = [], extra, pageType = 'WebPage' }) {
...
// ItemList of VisualArtwork for a gallery page (plan WS-E step 4). Rendered
// through the page's `jsonld` data by src/<gallery>/<gallery>.11tydata.js.
export function galleryStructuredData({ site, url, title, items }) {
  const absolute = (value) => new URL(value, site.url).href;
  return {
    '@type': 'ItemList',
    '@id': absolute(url) + '#items',
    name: title,
    ...
    itemListElement: items.map((item, index) => ({
      ...
      item: {
        ...
        image: absolute(item.full),
        thumbnailUrl: absolute(item.thumb),
        artMedium: 'Digital',
